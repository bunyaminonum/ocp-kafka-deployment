...
> authentication is PLAIN, SCRAM, or mTLS. It does **not** require SASL/SCRAM (which, as
> covered above, CFK doesn't support natively anyway).

Both CRs already carry `spec.authorization.type: simple` with `superUsers: [User:kafka]`.
Before this is set, Kafka has no authorization check at all — any authenticated SASL
principal (including `client`) can do anything. After it's set, **every principal except the
superUsers is denied by default** unless an explicit ACL grants it.

**Both CRs need `User:kafka` as a superuser, not just the Kafka CR.** ACL-management requests
(`kafka-acls --add`) are forwarded from the broker that receives them to the active KRaft
controller, which authorizes the request against **its own** superUsers list — not the Kafka
CR's. Leaving the KRaftController's `superUsers` empty (as shown in Confluent's own example)
causes `kafka-acls --add` to fail with `CREATE_ACLS needs ALTER permission`, even though
`kafka` is already a superuser on the Kafka side. Confirmed by testing, not by guessing —
verified against the actual `ConfluentServerAuthorizer` decision in the broker's request log.

Grant a principal access to a specific topic:
```bash
./scripts/grant-topic-acl.sh $NS client health-check
oc exec -n $NS kafka-0 -- kafka-acls --bootstrap-server kafka:9071 --command-config /tmp/admin-grant.properties --list
```

**Two easy-to-miss gotchas, both confirmed against real broker logs:**
- `kafka-topics --describe` needs **both** `Describe` and `DescribeConfigs` on the topic (it
  shows partition info *and* topic configs) — `Describe` alone isn't enough and fails with
  `Topic authorization failed`. `grant-topic-acl.sh` grants both.
- A principal with **zero** `Write` grants anywhere gets denied at topic-agnostic
  `Topic:ANY` scope (`ClusterAuthorizationException`) the moment it tries to produce — even
  to a topic it was never going to have `Write` on. This is Kafka's idempotent-producer
  default (KIP-679: producers are idempotent unless told otherwise) checking "does this
  principal have `Write` on *any* topic" before it even looks at the specific one. Not a bug —
  a principal that's only ever meant to read shouldn't be granted `Write` just to silence this.
